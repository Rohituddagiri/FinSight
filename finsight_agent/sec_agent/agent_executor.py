...
SEC_A2A_APP_NAME = "sec_a2a_app"


class SecADKAgentExecutor(AgentExecutor):
    ...
    def __init__(self, agent:Agent, agent_card:AgentCard, runner:Runner):
        ...
        Args:
            agent: The SEC ADK agent instance
            agent_card: Agent card for A2A service registration
            runner: Pre-configured ADK Runner instance
        """
        logger.info(f"Initializing SecADKAgentExecutor for agent: {agent.name}")
        self.agent = agent
    ...
    async def execute(self,
        ...
        """Execute the SEC agent's logic for a given request context.
    ...
    async def cancel(self,
        ...
        logger.info(
            f"Cancelling SEC search task: {task_id} for agent {self.agent.name}"
        )
        ...
        logger.info(f"Sent cancel event for SEC task: {task_id}")
    ...
    def _send_response(
        ...
        logger.info(f"Sending SEC search response for task {context.task_id}")
    ...
    def _handle_error(
        ...
        logger.error(
            f"Error executing SEC search in {self.agent.name}: {str(error)}",
            exc_info=True,
        )
        error_message_text = f"Error searching SEC workspace: {str(error)}"
        event_queue.enqueue_event(
            new_agent_text_message(
