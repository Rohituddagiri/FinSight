...
AGENT_URL = os.getenv("SEC_AGENT_A2A_URL", "http://localhost:8002")
...
async def run_sec_search_test(client: A2AClient) -> None:
    """Test the SEC agent's search functionality using the 'Nvidia' use case."""
...
async def run_sec_database_test(client: A2AClient) -> None:
    """Test the SEC agent's database query functionality, specifically counting."""
    await _execute_and_poll_task(
        client,
        "Get the AccountsReceivableNetCurrent fact data under us-gaap taxonomy for the CY2021Q1 frame for cik number 0001717115",
        "SEC Data Retrieval Test",
    )


async def main() -> None:
    """Main function to run SEC agent tests."""
    print(f"Connecting to SEC Agent at {AGENT_URL}...")
    try:
        async with httpx.AsyncClient(timeout=60.0) as httpx_client:
            ...
            await run_sec_search_test(client)
            await run_sec_database_test(client)
